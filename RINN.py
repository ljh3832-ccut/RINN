...
# Copyright (C) 2024 ccut. All Rights Reserved.
#
...
__all__ = ['RINN', 'rinn', 'reparameterize_model']
...
class RIB(nn.Module):
    ...
    def _fuse_bn_tensor(self, branch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if isinstance(branch, nn.Sequential):
            ...
            kernel = self.id_tensor # identity conv
...
class RINN(nn.Module):
    ...
    def __init__(self,
        ...
        # Build stages
        self.stage0 = RIB(in_channels=3, out_channels=self.in_planes,
    ...
    def _make_stage(self,
        ...
        """ Build a stage of rinn model.

        :param planes: Number of output channels.
        :param num_blocks: Number of blocks in this stage.
        :param num_se_blocks: Number of SE blocks in this stage.
        :return: A stage of rinn model.
        """
        ...
        for ix, stride in enumerate(strides):
            ...
            # Depthwise conv
            blocks.append(RIB(in_channels=self.in_planes,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.stage3(x)
        x = self.stage4(x)
        x = self.gap(x) # (b, c, 1, 1)   c: 512*a
        x = x.view(x.size(0), -1) # (b, c)
        x = self.linear(x) # (b, N)  N: number of classes
        return x
...
def rinn(num_classes: int = 1000, inference_mode: bool = False,
    ...
    return RINN(num_classes=num_classes, inference_mode=inference_mode,
                     **variant_params)
...
def reparameterize_model(model: torch.nn.Module) -> nn.Module:
    ...
    :param model: rinn model in train mode.
    :return: rinn model in inference mode.
    """
